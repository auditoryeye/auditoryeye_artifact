...
for cccc = 1:length(Cats_count)
    ...
    if length(tdoaarray_tmp) > 0
        % remove outliers
        

        [B1,I1] = rmoutliers(double(tdoaarray_tmp));
        C1 = C;
        C1(I1) = [];
        [B1,I1] = rmoutliers(B1,"percentiles",[10 90]);
        tdoaarray_tmp = B1;
        ...
        statistics(cccc,1) = realmax("single");
        statistics(cccc,2) = 0;
        statistics(cccc,3) = realmax("single");
        statistics(cccc,4) = realmax("single");

        statistics(cccc,5) = realmax("single");
        statistics(cccc,6) = 0;
        statistics(cccc,7) = realmax("single");
        statistics(cccc,8) = realmax("single");
...
for aa = 1:attempts
    for kk = 1:totalLen
        if (strcmp(keycellarray{kk},ResultsM(kk,aa)))
            revealed(kk) = 1;
        elseif(strcmp(keycellarray{kk},findupper(ResultsM(kk,aa))))
            revealed(kk) = 1
            keycellarray{kk}
            ResultsM(kk,aa)
        end
...
for kk = 1:totalLen
    result_tmp1 = keynametrim(results1{kk});
    result_tmp2 = keynametrim(results2{kk});
    result_tmp3 = keynametrim(results3{kk});
    truth_tmp = keynametrim(truth0{kk});
    label_label =  sprintf('%s',result_tmp1);
    if (strcmp(results1{kk},truth0{kk}) || strcmp(findupper(results1{kk}),truth0{kk}))
       textcolortmp = 'k';
       if strcmp(findupper(results1{kk}),truth0{kk})
            label_label =  sprintf('%s',truth_tmp);
       end
    elseif (strcmp(results2{kk},truth0{kk})|| strcmp(findupper(results2{kk}),truth0{kk}))
      	textcolortmp = 'b';
        label_label =  sprintf('%s',result_tmp2);
       if strcmp(findupper(results2{kk}),truth0{kk})
            label_label =  sprintf('%s',truth_tmp);
       end
    elseif (strcmp(results3{kk},truth0{kk})|| strcmp(findupper(results3{kk}),truth0{kk}))
      	textcolortmp = 'g';
        label_label =  sprintf('%s',result_tmp3);
       if strcmp(findupper(results3{kk}),truth0{kk})
            label_label =  sprintf('%s',truth_tmp);
       end
    else
       textcolortmp = 'r';
       label_label =  sprintf('%s(%s)',result_tmp1,truth_tmp);
    end
    text(double(tdoaarray(kk))*unit_scale,double(tdoa_v_array(kk))*unit_scale,label_label,'FontSize',11,'Color',textcolortmp, 'HorizontalAlignment', 'center','VerticalAlignment', 'middle');
